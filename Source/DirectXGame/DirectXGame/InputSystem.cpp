        ...
        for (unsigned int i = 0; i < 256; i++)
        {
            ...
            if (m_keys_state[i] & 0x80)
            {
                ...
                while (it != m_set_listeners.end())
                {
                    if (i == VK_LBUTTON)
                    {
                        m_keys_state[i] != m_old_keys_state[i]; (*it)->onLeftMouseButtonDown(Point(current_mouse_pos.x, current_mouse_pos.y));
                    }
                    else if (i == VK_RBUTTON)
                    {
                        m_keys_state[i] != m_old_keys_state[i]; (*it)->onRightMouseButtonDown(Point(current_mouse_pos.x, current_mouse_pos.y));
                    }
                    else
                        (*it)->onKeyDown(i);
                    ++it;
